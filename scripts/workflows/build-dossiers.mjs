...
- ACCURACY — WEAPON HARDPOINTS (do not hallucinate): before writing ANY claim that the ship is "gunless / weaponless / unarmed / has no weapon hardpoints / can't fight", read data/ships/<hull>.json slots.hardpoints. Each entry 1-4 is a weapon hardpoint (1=S,2=M,3=L,4=H); 0s are utility mounts. Almost every medium and large hull — INCLUDING liners (Beluga, Orca, Dolphin) and freighters — HAS weapon hardpoints and can mount deterrent guns. Only claim "no weapons" if slots.hardpoints is ALL zeros (extremely rare). Report the true hardpoint count in §02 (e.g. "5 Medium").
