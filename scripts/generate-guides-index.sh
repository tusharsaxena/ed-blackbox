...
cat <<'FOOT'
  ...
  <section id="section-faq">
    ...
    <div class="cards one">
      <div class="card"><h4>Is this affiliated with Frontier Developments?</h4><p>No. The Black Box is unofficial fan content &mdash; not affiliated with, sponsored by, or endorsed by Frontier Developments. <em>Elite Dangerous</em> is a trademark of Frontier Developments plc, and all game data, names and imagery remain their property.</p></div>
      <div class="card"><h4>Is this a complete guide to the game?</h4><p>No. The Black Box covers only the ships, systems and activities I've worked through myself so far &mdash; not the whole of Elite Dangerous. I add guides as I get hands-on with more of the game, so the library keeps growing; expect gaps, and expect them to fill in over time. Want something specific covered next? <a href="https://github.com/tusharsaxena/ed-blackbook/issues">Open a feature request</a>.</p></div>
      <div class="card"><h4>Does this apply to my platform (PC / console)?</h4><p>Yes. Ship stats, modules and engineering are the same on PC and console, so the ratings and builds hold wherever you play.</p></div>
      <div class="card"><h4>How current is the information?</h4><p>Game figures &mdash; ship, module and engineering stats &mdash; are checked against EDCD/coriolis-data, INARA, EDSY and Coriolis, and every page lists its sources at the foot. Ship ratings, loadouts and &ldquo;best for the job&rdquo; picks are <em>editorial judgement</em>, not Frontier data. When the game changes a page can lag; anything uncertain is marked rather than guessed, and corrections are welcome.</p></div>
      <div class="card"><h4>How are the ship ratings decided?</h4><p>Each ship gets a 1&ndash;100 score for a given role, but it's a judgement call, not a spec-sheet sum. The number is <em>roster-relative</em> (measured against every other ship doing that job), assumes a <em>fully-engineered</em> build, and is weighed across an ordered set of role factors. Every dossier shows the working in its &ldquo;Why This Rating&rdquo; scorecard, and the full method is on the <a href="ships/rating-methodology.html">rating methodology</a> page.</p></div>
      <div class="card"><h4>Why is the same ship rated differently in different roles?</h4><p>Because each score is set against the field <em>for that role</em>, using that role's factor weights. A hull that's a monster hauler can be middling in a knife-fight, so its trading and combat numbers differ on purpose. Compare ships within a role, not across roles.</p></div>
      <div class="card"><h4>Can I open these builds in Coriolis or EDSY?</h4><p>Yes. Every dossier's 3-State Loadout has <strong>Open in Coriolis</strong> and <strong>Open in EDSY</strong> links that load the exact build straight into the planner &mdash; no copy-paste &mdash; plus a <strong>Copy SLEF</strong> button to drop it into anything else that reads Ship Loadout Export Format.</p></div>
      <div class="card"><h4>What do the three loadout columns mean?</h4><p>They're a budget path, not one fixed build. <strong>Initial</strong> is buy-only with no engineering &mdash; what you fly the day you buy the hull. <strong>A-Rated</strong> swaps in A-rated cores for a combat-ready ship. <strong>Engineered</strong> is the full blueprint-and-experimental tour. Fly whichever column you can afford and work toward the next.</p></div>
      <div class="card"><h4>Do I have to engineer everything?</h4><p>No. The A-Rated column is a genuinely capable ship on its own &mdash; engineering is the ceiling, not the price of entry. Engineer when you're ready; each dossier's plan tells you what to prioritise first.</p></div>
      <div class="card"><h4>Why isn't a particular ship or role covered?</h4><p>The dossiers cover the ship &times; role pairings actually worth flying. If a hull can't do a job well enough to recommend, it's left out rather than padded with a build no one should fly.</p></div>
      <div class="card"><h4>Is this AI-generated content?</h4><p>Yes &mdash; although I prefer to call it <em>curated</em> AI content. I steer the model deliberately to write the guides I want, including fact-checking against authoritative sources. It began as a side project to learn Elite Dangerous's systems for myself; as the collection of personal guides grew, it became something worth sharing. I've done my best to credit every source used &mdash; if your work isn't credited, <a href="https://github.com/tusharsaxena/ed-blackbook/issues">open an issue</a> and I'll make it right.</p></div>
      <div class="card"><h4>Found an error, or want to contribute?</h4><p>This is a personal project by CMDR Ka0s. Spot a wrong number, a broken link, or a build you'd fit differently? <a href="https://github.com/tusharsaxena/ed-blackbook/issues">Open an issue</a>. Ratings and loadouts are opinions &mdash; disagree and say why; a well-argued correction gets folded in.</p></div>
    </div>
  </section>
