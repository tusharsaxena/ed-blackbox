...
and (when a dossier exists) a deep link to guides/ships/dossiers/<ship>-<role>.html.
...
Modes:
  ...
  --page      write the real page to guides/ships/ship-role-matrix.html
    ...
    for s in ships:
        ...
        for key, label in ROLES:
            ...
            if c:
                ...
                if c["dossier"]:
                    inner = f'<a href="dossiers/{c["dossier"]}" title="{s} · {label}">{inner}</a>'
                else:
...
def main():
    ...
    if "--mockups" in sys.argv:
        ...
        out.write_text(page_shell(render_table(data), len(data), mockup=True))
        print(f"  wrote {out.relative_to(ROOT)}")
    if "--page" in sys.argv:
        out = OUT_DIR / "ship-role-matrix.html"
