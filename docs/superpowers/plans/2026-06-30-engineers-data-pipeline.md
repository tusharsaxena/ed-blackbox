...
- [ ] **Step 1:** Write the seeder (mirror `extract-blueprint-editorial.py`): parse each `<article … id="engineer-<slug>">`, capturing every field region **verbatim as HTML** (refers-to, unlock, notes, desc, **and Modifications offered** — keep inner HTML incl. `<a>`/`<li>`), the chips, banner src→slug, location kv, and section (from the enclosing `<section id>`), order = document order. **Per the preserve-and-verify decision, store `Modifications offered` for ALL 38 engineers** (ship + on-foot) verbatim — coriolis is a verifier, not a generator. (No separate `onfoot_mod_lists` needed; on-foot mods live in the same `mods_html` field.)
...
- [ ] **Step 1:** Write the generator (reuse materials' multi-marker `splice`, ordering blocks by section then `order`). `render_card(slug, ed)` reproduces the `<article …>` markup byte-compatibly **purely from `editorial.json`** (passthrough of preserved inner HTML): banner, name+permalink, chips, Location kvgrid, Meeting, **Modifications offered** (`mods_html` verbatim), Refers-to, Unlock, optional Notes, In-Game Description. One BEGIN/END pair per section; cards within a section concatenated. (Coriolis is not consulted here — only in the audit.)
...
- [ ] **Step 1:** Write the audit: (a) 38 cards, ids = `engineer-<slug>` for all 38 roster names; (b) marker pairs = 8; (c) **over-claim check (hard fail):** every rendered ship-engineer `G<n> <group>` whose group exists in coriolis must have `n <= max grade coriolis backs` for that engineer; (d) **omission report (warning, non-failing):** list engineer→group pairs coriolis shows but the page omits (for editorial review — many are deliberate variant-collapses); (e) every `blueprints.html#blueprint-group-<slug>` + `#engineer-<slug>` anchor resolves; (f) Sources external-only; (g) roster names equal `engineers.csv` (post-fix). The audit exits nonzero only on hard fails (a/b/c/e/f/g); warnings (d) print but pass.
